...
def simple_resume_parser(file_path):
    ...
    try:
        ...
        text = ""
    # try counting pages using form-feeds; fallback 1
    no_of_pages = 1
    if text:
        # pdfminer inserts form-feed '\f' between pages sometimes
    ...
    # Extract phone (several patterns)
    mobile_number = None
    phone_match = re.search(r'(\+?\d{1,3}[\s-]?)?(\d{10}|\d{3}[\s-]\d{3}[\s-]\d{4}|\(\d{3}\)\s*\d{3}[\s-]*\d{4})', text)
    if phone_match:
        mobile_number = phone_match.group().strip()
    # Extract name: first non-empty line that isn't email/phone heuristically
    name = ""
    lines = [ln.strip() for ln in text.splitlines() if ln.strip()]
    for ln in lines[:6]:  # check first few lines
        if email and email in ln: 
            continue
        if mobile_number and mobile_number in ln:
            continue
        # simple heuristic: name usually short and alphabetic
        if 2 <= len(ln.split()) <= 4 and re.search(r'[A-Za-z]', ln):
            name = ln
            break
    if not name and lines:
        name = lines[0]
    # Skills detection (simple keyword list — you can extend)
    skill_keywords = ["python", "c", "c++", "java", "sql", "machine learning", "deep learning",
                      "excel", "power bi", "tensorflow", "keras", "pytorch", "react", "django",
                      "flask", "android", "kotlin", "swift", "figma"]
    skills_found = []
...
def fetch_yt_video(link):
    """
    Safely fetches YouTube video info (title) using yt_dlp.
    Returns a short fallback string if extraction fails.
    """
    try:
        ...
        print("YT fetch error:", e)
        # fallback: return the last part of the URL (or a placeholder)
        try:
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'mlproject' and ad_password == 'ml123':
                st.success("Welcome Lakshya Shukla")
                # Display Data
